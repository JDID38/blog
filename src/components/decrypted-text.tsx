...
export default function DecryptedText({
  ...
  const [display, setDisplay] = useState<string[]>(() => buildDisplay(src, pool, new Set()))
  ...
  const lockNow = lockedIndices(src, revealedRef.current, fromStart)
  ...
  return (
    ...
    >
      {display.map((ch, i) => {
        const orig = src[i]
        const isSpace = orig === " "
        const isLocked = isSpace || lockNow.has(i)
